...
def train_pinn_eikonal(
    ...
    """
    Entrena la PINN para:

    - Isotrópico:
        |∇T| = s0

    - Anisotrópico:
        ∇T^T G ∇T = 1     (G viene precomputado por elemento)
    """
    device = device_auto()
    ...
    if source_nodes.size == 0:
        print("[WARN] No se han proporcionado nodos fuente: la solución quedará definida salvo una constante.")
    ...
    if anisotropic:
        ...
        if s0 != 1.0:
            print("[INFO] Modo anisotrópico: s0 se ignora; la ecuación es ∇T^T G ∇T = 1.")
    ...
    for ep in range(1, epochs + 1):
        ...
        T_int = model(x_int)                        # (B,1)
        gT = grad_t(T_int, x_int)                  # (B,3)
        if anisotropic:
            # grad T^T G grad T = 1
            elem_ids_torch = torch.as_tensor(elem_ids_np, dtype=torch.long, device=device)
            G_batch = G_elems_torch[elem_ids_torch]  # (B,3,3)
            quad = torch.einsum("bi,bij,bj->b", gT, G_batch, gT).unsqueeze(1)  # (B,1)
            res_pde = quad - 1.0
            loss_pde = torch.mean(res_pde**2)
            mean_grad_like = torch.sqrt(torch.clamp(quad, min=0)).mean().item()
        else:
            # |∇T| = s0
            grad_norm = torch.linalg.norm(gT, dim=1, keepdim=True)  # (B,1)
            s_val = s0 * torch.ones_like(T_int)
            res_pde = grad_norm - s_val
            loss_pde = torch.mean(res_pde**2)
            mean_grad_like = grad_norm.mean().item()

        # --- Contorno T=0 en fuentes ---
        ...
        # --- Regularizador de no negatividad ---
        ...
        if ep % 500 == 0 or ep == 1:
            with torch.no_grad():
                mean_Tsrc = loss_bc.sqrt().item() if source_nodes.size > 0 else 0.0
            print(
                f"[{ep:6d}] loss={loss.item():.4e} "
                f"(pde={loss_pde.item():.4e}, bc={loss_bc.item():.4e}, mono={loss_mono.item():.4e}) "
                f"||grad||_eff≈{mean_grad_like:.3f}, T_src_rms≈{mean_Tsrc:.3e}"
            )
